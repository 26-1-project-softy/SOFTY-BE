...
- `evaluationId` (String, optional): AI 평가 작업 ID (예: `eval_20260326_001`)
  - 값이 있으면 해당 ID로 조회
  - 값이 없으면 DB(`model_training_history`)에서 최신 `evaluation_id`를 조회해 사용

### Headers
- `Authorization`: `Bearer {JWT}`

## 동작 규칙
- AI 서버 호출 경로: `{AI_SERVER_BASE_URL}/ai/evaluations/{evaluationId}`
- `AI_SERVER_BASE_URL`는 필수 환경변수(기본값 없음)
- `evaluationId` 미입력 시 최신 `evaluation_id` 결정 기준
  - `evaluation_id IS NOT NULL AND TRIM(evaluation_id) <> ''`
  - `trained_at DESC, id DESC`
...
- `403 Forbidden`
  - 관리자 권한 아님
- `404 Not Found`
  - `evaluationId` 미입력 + `model_training_history`에 사용 가능한 `evaluation_id` 없음
- `502 Bad Gateway`
  - AI 서버가 4xx/5xx로 응답
  - AI 서버 응답 바디가 비어 있음
- `504 Gateway Timeout`
