...
4. 메시지는 최신 메시지부터 조회한 뒤, 응답에서는 오래된 순서로 정렬해 내려준다.
5. `cursor`는 이전 응답의 마지막 메시지 ID이다.
6. 다음 조회 시에는 `cursor`보다 더 오래된 메시지만 조회한다.
7. `isMine`은 로그인한 사용자가 보낸 메시지인지 여부이다.
8. `senderName`, `senderRole`은 메시지 발신자 기준 값이다.
    ...
    "messages": [
      {
        ...
        "senderName": "김민수",
        "senderRole": "PARENT",
        "content": "선생님, 내일 철수가 병원 진료가 있어 조금 늦을 것 같습니다.",
...
- 읽음 처리 정책은 별도 API 또는 상세 진입 시점 정책으로 분리해 관리할 수 있다.
