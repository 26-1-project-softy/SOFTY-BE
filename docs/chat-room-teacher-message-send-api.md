...
6. `analysisId`에 해당하는 메시지 분석 결과가 존재하는지 확인한다.
7. 해당 분석 결과의 작성자와 로그인 교사가 동일한지 확인한다.
8. 해당 분석 결과가 현재 `chatRoomId`와 연결된 분석 결과인지 확인한다.
9. 최종 메시지를 저장한다.
10. 분석 결과의 추천문장과 최종 전송 문장을 비교해 추천문장 채택 여부를 기록한다.
11. 분석 결과와 최종 전송 메시지를 연결할 수 있도록 관련 이력을 저장한다.
12. 상대방의 `unreadCount`를 증가시킨다.
13. 생성된 메시지 정보를 반환한다.
